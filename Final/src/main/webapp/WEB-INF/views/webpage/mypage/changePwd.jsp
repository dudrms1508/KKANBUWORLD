...
<body>
    <div id="wrapdiv">
        <div id="wrapleftdiv">
            <div id="categorydiv">
                <ul id="categoryul">
                    <li id="myInformation"><button onclick="location.href='${ path }/webpage/mypage/myInfo'">내정보</button></li>
                    <li id="passwordchg"><button style="border-bottom: 2px solid rgb(243, 156, 18); color: rgb(243, 156, 18); cursor: default;">비밀번호변경</button></li>
                    <li id="deleteAccount"><button onclick="location.href='${ path }/webpage/mypage/accountdel'">회원탈퇴</button></li>
                </ul>
            </div>
            <div id="privatetextdiv">
                <span><b>비밀번호를 변경하려면 현재 비밀번호를 입력해주세요.</b></span>
                <span style="color: rgb(243, 156, 18);"></span>
            </div>
            <form action="${ path }/webpage/mypage/changePwd" method="post" id="tableformdiv">
                <div id="rectanglediv">
                    <div id="tablediv">
                        <table>
                            <tr>
                            ...
                            <tr>
                                <td class="tdbold">현재 비밀번호</td>
                                <td><input type="password" name="password" id="pwd" class="inputchg" placeholder="현재 비밀번호를 입력하세요." required></td>
                            </tr>
                            <tr>
                                <td class="tdbold">변경할 비밀번호</td>
                                <td><input type="password" name="chgPwd" id="chgPwd" class="inputchg" placeholder="변경할 비밀번호를 입력하세요." required></td>
                            </tr>
                            <tr>
                                <td class="tdbold">비밀번호 확인</td>
                                <td><input type="password" name="chgPwd2" id="chgPwd2" class="inputchg" placeholder="변경할 비밀번호를 재입력하세요." required></td>
                            </tr>
                        </table>
                    </div>
                </div>
                ...
                <div id="btmbtndiv">
                    ...
                    <input type="submit" id="chgbtn" value="변경하기"></input>
                </div>
            </form>
        </div>
...
window.onload = function() {
    ...
    // 비밀번호 재입력 확인
    let chgPwdInput = document.getElementById('chgPwd');

    chgPwdInput.addEventListener('keyup', () => {
        let chgpwd = document.getElementById('chgPwd').value;
        let chgPwd2 = document.getElementById('chgPwd2').value;
        let pwdRegExp = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,12}$/;
        
        if(!pwdRegExp.test(chgpwd)) {
            chgReady = false;
            return false;
        } else if(chgpwd != chgPwd2) {
            chgReady = false;
            return false;
        } else {
            chgReady = true;
    ...
    chgbtn.addEventListener('click', (event) => {
        ...
        if(!chgReady) {
            alert('대/소문자, 특수문자, 숫자 각 하나씩 포함하여 6~12글자 사이로 입력해 주세요.');
            event.preventDefault();
        } else if (pwd == chgPwd) {
            alert('현재 비밀번호와 다른 비밀번호로 변경해주세요.')
            event.preventDefault();
        }
    });
